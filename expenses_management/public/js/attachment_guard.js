 ...
 *
 * Rules:
 *  - Sales Invoice is fully excluded from the guard.
 *  - .xsl, .xml, and .png files are always exempt (can be attached/removed anytime).
 *  - All other file types are locked on submitted documents.
 */
(function () {
	...
	function is_ignored_doctype(frm) {
		return frm && IGNORED_DOCTYPES.includes(frm.doctype);
	}
	...
	Attachments.prototype.new_attachment = function (fieldname) {
		if (is_attachment_locked(this.frm)) {
			// Locked — restrict to exempt file types only
			if (this.dialog) {
